...
class IrUiMenu(models.Model):
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        ...
        try: #Hide a menu based on the value in company master
            # my_leave_request_menu = self.env.ref('hr_holidays_saudi.hr_holidays_saudi')
            # d_mgr_leave_req_approval_menu = self.env.ref('hr_holidays_saudi.hr_holidays_saudi_direct')
            # team_mgr_leave_req_approval_menu = self.env.ref('hr_holidays_saudi.hr_h"olidays_saudi_team')
            # all_leave_request_approval_menu = self.env.ref('hr_holidays_saudi.hr_holidays_saudi_hr')
            # # no use
            # officer_leave_request_approval_menu = self.env.ref('hr_holidays_saudi.hr_holidays_saudi_officer')
            # gm_leave_request_approval_menu = self.env.ref('hr_holidays_saudi.hr_holidays_saudi_hr_gm')
            #
            ...
            # print ('is_coachis_coach',is_coach)
            # if not is_coach:
